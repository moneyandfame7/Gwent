...
struct CombatRowView: View {
    ...
    private var isSelectable: Bool {
        ...
        guard let selectedCard = viewModel.ui.selectedCard else {
            return false
        }
    ...
    var hornView: some View {
            ...
            if let horn = row.horn {
                CardView(card: horn)
                    .matchedGeometryEffect(
                        id: horn.id,
                        in: viewModel.ui.namespace(isMe: isMe)
                    )
            }
        }
        .frame(width: 80)
        ...
        .onTapGesture {
            ...
            Task {
                await viewModel.playCard(
                    viewModel.ui.selectedCard!,
                    row: row.type
                )
            }
        }
            ...
            ZStack {
                ...
                HStackOfCards(row.cards, id: \.id) { card in
                    CardView(card: card, isPlayable: true, size: .extraSmall)
                        .matchedGeometryEffect(id: card.id, in: viewModel.ui.namespace(isMe: isMe))
                }
            }
        }
